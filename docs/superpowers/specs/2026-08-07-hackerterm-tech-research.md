...
| **插件 API 是独立一层，不直接暴露核心协议** | 核心协议要能自由演进（§7 的 major/minor 规则），直接开放给插件会让改协议就破坏所有插件 |
| **声明式清单 + 贡献点 + 激活事件** | 照抄 VS Code：不加载插件代码就能渲染它的命令/菜单/快捷键；用不到的插件根本不加载 |

### ⚠️ V1 就要做的准备：命令 / 菜单 / 快捷键 / 主题 / 面板类型必须「可注册」

「贡献点」机制要求这些东西是**注册表驱动**的，不是硬编码在组件里。

| V1 要做成 | 而不是 |
|---|---|
| 命令注册表，命令面板从表里读 | 命令面板里写死一个数组 |
| 菜单项注册表，右键菜单从表里读 | 每个组件自己写 `<MenuItem>` |
| 快捷键注册表（本来就要做管理器） | 组件里散落 `onKeyDown` 判断 |
| 主题注册表 | 只认内置那两套 |
| 面板类型注册表 | `switch (tabKind)` 写死四种 |

**这是本次参照里最容易漏、但事后代价最大的一条。**
V1 硬编码，将来做插件时要动整个 UI 层。现在做成注册表，几乎不多花时间。
