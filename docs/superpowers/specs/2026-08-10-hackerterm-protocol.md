...
| # | 待定 | 影响 |
|---|---|---|
| 1 | ~~Arrow 数据块在数据通道上的分帧格式~~ **已定稿，见下** | 数据库大结果集链路 |
...
**采用 Arrow 的 IPC Stream 格式，不额外套长度前缀。**

Rust 侧用 `arrow::ipc::writer::StreamWriter` 编码、`arrow::ipc::reader::StreamReader`
解码（`crates/ht-core/src/arrow_frame.rs`）；渲染进程侧用 `apache-arrow` 的
`tableFromIPC` 解码。IPC Stream 格式自带 schema 消息和记录边界，帧体本身就是
自描述、自定界的，不需要我们在数据通道上再套一层长度前缀。

**实测数据**（`bench/arrow.ts`，10 万行 x 10 列，5 个 `Int64` 列 + 5 个可空
`Utf8` 列，贴近真实查询结果的列类型分布，3 次独立运行）：

| 环节 | 耗时 / 体积 | 备注 |
|---|---|---|
| Rust 侧编码（`encode_batch`） | 约 31～53 ms | `cargo run --release` 下实测，3 次运行区间 |
| IPC 传输（MessagePort，核心 → 渲染进程） | **未测量** | 容器内无法启动沙箱化 Electron（root 跑 Electron 需 `--no-sandbox`，与本项目渲染进程必须保持 OS 沙箱的硬约束冲突），**留给 Windows 真机验证** |
| JS 侧解码（`apache-arrow` 的 `tableFromIPC`，纯 Node 环境） | 约 14～18 ms | |
| 字节体积 | 13,571,592 bytes（约 12.94 MiB） | |
| 总计（仅编码 + 解码，不含传输） | 约 45～71 ms | |

判据：**总计应在百毫秒量级，超过 1 秒说明分帧方式有问题**。实测的编码 + 解码
总计在百毫秒以内，满足判据；缺 IPC 传输这一段实测，结论暂定为「大概率没问题」，
正式关闭此项前仍需要 Windows 真机上跑一次完整往返（含 MessagePort 传输）确认。
